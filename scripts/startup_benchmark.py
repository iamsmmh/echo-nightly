...
def launch_time(output, expected_package=None):
    if not re.search(r'^Status:\s*ok\s*$', output, re.M):
        raise ValueError('Android launch did not succeed: ' + output[:2000])
    if not re.search(r'^LaunchState:\s*COLD\s*$', output, re.M):
        raise ValueError('Launch was not process-cold: ' + output[:2000])
    if expected_package and not re.search(r'^Activity:\s*' + re.escape(expected_package) + r'/[^\r\n]+$', output, re.M):
        raise ValueError('Launch measured a different activity: ' + output[:2000])
...
def main():
    ...
    adb('install', '-r', '-t', '-g', str(candidates[0]))
