...
abstract class BaseTask(
    ...
    suspend fun doWork() = withContext(Dispatchers.IO) {
        ...
        if (throwable == null && download != null) {
            // Phase 1 (stability): record a checksum sidecar for every finished
            // download so corruption can be detected before playback.
            download.finalFile?.let {
                dev.brahmkshatriya.echo.utils.FileIntegrity.record(File(it))
            }
        }
        if (throwable != null && download != null) {
