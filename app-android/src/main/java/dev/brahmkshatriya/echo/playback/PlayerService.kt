...
import dev.brahmkshatriya.echo.playback.cast.ChromecastBridge
...
import dev.brahmkshatriya.echo.utils.PlaybackRecoveryStore
import dev.brahmkshatriya.echo.playback.ResumptionUtils.recoverPlaylist
import dev.brahmkshatriya.echo.wear.WearBridge
...
class PlayerService : MediaLibraryService() {
    ...
    private var watchdog: PlaybackWatchdog? = null
    private var wearBridge: WearBridge? = null
    private var castBridge: ChromecastBridge? = null
    ...
    override fun onCreate() {
        ...
        // Phase 1 (stability): playback watchdog + crash-safe resume.
        watchdog = PlaybackWatchdog(
            player = exoPlayer,
            settings = app.settings,
            scope = scope,
            onReloadItem = { p ->
                PlaybackWatchdog.reloadItem(p) { item -> MediaItemUtils.withRetry(item) }
            },
            onSkipTrack = { p -> p.seekToNextMediaItem() }
        ).also { it.start() }
    ...
    /**
     * When the process was killed mid-playback (crash or memory pressure),
     * restore the queue + position and resume once the service is back up.
     * If a UI controller connects it takes over the resume itself, so we hold
     * off as soon as one shows up.
     */
    @OptIn(UnstableApi::class)
    private fun recoverPlaybackAfterCrash(session: MediaLibrarySession) {
        scope.launch(Dispatchers.Main) {
            delay(2_000)
            val snapshot = PlaybackRecoveryStore.load(this@PlayerService) ?: return@launch
            if (!snapshot.wasPlaying) return@launch
            if (!app.settings.getBoolean(PlaybackRecoveryStore.AUTO_RESUME_KEY, true)) {
    ...
    /** Applied by [dev.brahmkshatriya.echo.wear.EchoWearListenerService] via [WearBridge]. */
    @OptIn(UnstableApi::class)
    internal fun switchPlayerQuality(serverIndex: Int, loaded: Boolean) {
        val player = mediaSession?.player ?: return
        val item = player.currentMediaItem ?: return
        val rebuilt = MediaItemUtils.buildServer(item, serverIndex)
        val finalItem = if (loaded) rebuilt else MediaItemUtils.withRetry(rebuilt)
        player.replaceMediaItem(player.currentMediaItemIndex, finalItem)
        player.prepare()
        player.play()
    }

    override fun onDestroy() {
        runCatching { PlaybackRecoveryStore.save(this, mediaSession?.player ?: exoPlayer) }
        watchdog?.stop()
        watchdog = null
        wearBridge?.stop()
        wearBridge = null
