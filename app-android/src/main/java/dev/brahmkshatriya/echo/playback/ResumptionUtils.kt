...
object ResumptionUtils {
    ...
    fun Context.recoverTracks(withClear: Boolean = false): List<Pair<MediaState.Unloaded<Track>, EchoMediaItem?>>? {
        if (withClear && getFromCache<Boolean>(CLEARED, maxAgeMillis = Long.MAX_VALUE) != false) return null
        val tracks = getFromCache<List<Track>>(TRACKS, FOLDER, Long.MAX_VALUE)
        val extensionIds = getFromCache<List<String>>(EXTENSIONS, FOLDER, Long.MAX_VALUE)
        val contexts = getFromCache<List<EchoMediaItem>>(CONTEXTS, FOLDER, Long.MAX_VALUE)
    ...
    fun Context.recoverIndex() = getFromCache<Int>(INDEX, FOLDER, Long.MAX_VALUE)
    private fun Context.recoverPosition() = getFromCache<Long>(POSITION, FOLDER, Long.MAX_VALUE)


    fun Context.recoverShuffle() = getFromCache<Boolean>(SHUFFLE, FOLDER, Long.MAX_VALUE)
    fun saveShuffle(context: Context, shuffle: Boolean) {
        context.saveToCache(SHUFFLE, shuffle, FOLDER)
    }

    fun Context.recoverRepeat() = getFromCache<Int>(REPEAT, FOLDER, Long.MAX_VALUE)
