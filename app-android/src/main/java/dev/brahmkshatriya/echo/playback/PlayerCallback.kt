...
class PlayerCallback(
    ...
    private fun onSleepTimer(player: Player, ms: Long): ListenableFuture<SessionResult> {
        ...
        timerJob = scope.launch {
            // Phase 5 (audio): linear volume fade over the last 15s, mirroring
            // the shared SleepStateMachine curve of the KMP player.
            val fadeMs = minOf(15_000L, time / 4).coerceAtLeast(0)
            val fadeStart = time - fadeMs
            if (fadeStart > 0) delay(fadeStart)
            if (fadeMs > 0) {
                val steps = 10
                val stepDelay = (fadeMs / steps).coerceAtLeast(1)
                repeat(steps) { i ->
                    val factor = 1f - (i + 1) / steps.toFloat()
                    runCatching { player.with { volume = factor } }
                    delay(stepDelay)
                }
            }
            runCatching {
