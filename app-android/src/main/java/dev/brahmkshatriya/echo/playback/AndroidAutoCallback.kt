...
abstract class AndroidAutoCallback(
    ...
    override fun onGetChildren(
        ...
        when (type) {
            ...
            HOME -> extension.getFeed<HomeFeedClient>(
                context, "$ROOT/$extId/$HOME", HOME, page, extId
            ) { loadHomeFeed() }

            LIBRARY -> extension.getFeed<LibraryFeedClient>(
                context, "$ROOT/$extId/$LIBRARY", LIBRARY, page, extId
            ) { loadLibraryFeed() }

            SEARCH -> extension.getFeed<SearchFeedClient>(
                context, "$parentId#$searchQuery", SEARCH, page, extId
            ) { loadSearchFeed(searchQuery) }

            FEED -> extension.getList<ExtensionClient> {
                // Browsing "more" on a shelf from home/library/search feeds.
                val feedId = parentId.substringAfter("$FEED/")
                val feed = feedMap[feedId] ?: return@getList emptyList()
                feed.toMediaItems(feedId, context, extId, page)
            }
    ...
    companion object {
        ...
        private fun Track.toItem(
            ...
            context.saveToCache("$extensionId/$id", Triple(this, extensionId, con), "auto")
            return MediaItem.Builder()
                .setMediaId("auto/$extensionId/$id")
                .setMediaMetadata(
                    MediaMetadata.Builder()
                        .setIsPlayable(true)
        ...
        private suspend fun Feed<Shelf>.toMediaItems(
            ...
            val key = id
            feedMap[key] = this
            // Phase 7 (Android Auto): real paging through the feed's shelves.
            val paged = getPagedData(tabs.firstOrNull()).pagedData
            val (list, next) = paged.loadPage(continuations[key to page])
        ...
        private suspend fun getTracks(
            ...
            val (item, tracks) = tracksMap.getOrPut("$extId/$id") {
            ...
            val (list, next) = tracks.loadPage(continuations["$extId/$id" to page])
            continuations["$extId/$id" to page + 1] = next
