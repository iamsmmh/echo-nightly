...
import dev.brahmkshatriya.echo.player.core.withRetry
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.launch
import java.io.IOException

private fun Throwable.isNetworkish(): Boolean =
    generateSequence(this as Throwable) { it.cause }.any {
        it is IOException || it is java.net.UnknownHostException ||
            it is java.net.SocketTimeoutException || it is javax.net.ssl.SSLException
    }
...
class StreamableMediaSource(
    ...
    override fun prepareSourceInternal(mediaTransferListener: TransferListener?) {
        ...
        scope.launch {
            // Phase 1 (stability): transient network failures while resolving
            // streams are retried automatically with exponential backoff.
            var (new, serv) = runCatching {
                withRetry(
                    RetryPolicy.Playback,
                    shouldRetry = { err, _ -> err.isNetworkish() }
                ) { attempt ->
                    if (attempt > 1) error = null
                    loader.load(mediaItem)
                }
            }.getOrElse {
                error = it
                return@launch
