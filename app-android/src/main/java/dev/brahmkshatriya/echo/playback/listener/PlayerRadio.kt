...
class PlayerRadio(
    ...
    companion object {
        ...
        suspend fun play(
            ...
            val tracks = loaded.tracks(loaded.cont)
            if (tracks == null) {
                // Phase 1 (stability): never leave the UI stuck on "Loading".
    ...
    private suspend fun loadPlaylist() = dev.brahmkshatriya.echo.player.domain.runCatchingCancellable {
        ...
        // A misbehaving extension must not crash playback; radio just stops.
        stateFlow.value = PlayerState.Radio.Empty
        scope.launch { throwFlow.emit(it) }
    }
