...
class StreamableLoader(
    ...
    private suspend fun loadServer(mediaItem: MediaItem): Result<Streamable.Media.Server> {
        ...
        if (!downloaded.isNullOrEmpty() && servers.size == index) {
            // Phase 1 (stability): never trust a download blindly. Verify size +
            // recorded checksum (sidecar) and the audio header before using it.
            // Anything corrupted falls back to streaming transparently.
            val files = downloaded.map { File(it) }
            val corrupted = files.filter { !it.exists() || it.length() <= 0
                    || dev.brahmkshatriya.echo.utils.FileIntegrity.verify(it).isFailure }
            if (corrupted.isEmpty()) {
                ...
                "EchoLoader", "corrupted downloads, falling back to stream: " +
                    corrupted.joinToString { it.name }
            )
        }
